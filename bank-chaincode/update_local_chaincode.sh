#! /bin/bash
# When updating the chaincode locally (Just on your peer), you don't need other peers to approve & commit the updated chaincode
# Only this peer has to approve, no commit is needed

CC_NAME=simpleTx
# Note that CC_VERSION and SEQUENCE remains unchanged, as this is just a local update, and not a network update that requires other peers to approve 
CC_VERSION=1.0
# Update the label version for version control purposes
CC_LABEL=${CC_NAME}_2.0
CC_PACKAGE_FILE=${CC_LABEL}.tar.gz

# For local update, the sequence can be the same 
SEQUENCE=1
